...
export default function CircuitsList() {
  useEffect(() => {
    const elements = document.querySelectorAll<HTMLElement>('.liquid-glass-strong');
    const effects = Array.from(elements).map(el => 
      liquidGlass(el, {
        scale: -112,
        chroma: 6,
        border: 0.07,
          ...
          <div className="relative z-10 px-6 md:px-16 lg:px-20 pt-24 pb-24 flex flex-col min-h-screen pointer-events-none">
            ...
            >
              {circuits.map((circuit) => (
                ...
                className="liquid-glass-strong p-8 flex flex-col group transition-transform duration-300 hover:-translate-y-1 focus-visible:ring-2 focus-visible:ring-white focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-black"
              >
